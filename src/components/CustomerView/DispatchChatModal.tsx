...
export const DispatchChatModal: React.FC<DispatchChatModalProps> = ({ category, onClose }) => {
  const { currentUser, initiateCall, updateUserLocally, globalRefreshKey } = useAuth();
  ...
  // Quick issue suggestions by category
  const getQuickIssues = (catId?: string) => {
    switch (catId) {
      case 'cat-electrician':
        return ['Circuit breaker tripped', 'Sparking wall outlet', 'EV charger installation', 'Power outage in room'];
      case 'cat-plumbing':
        return ['Burst water pipe', 'Clogged kitchen sink', 'Running toilet repair', 'Low water pressure'];
      case 'cat-cleaners':
        return ['Deep house cleaning', 'Move-out sanitizing', 'Carpet & upholstery', 'Kitchen & oven degrease'];
      case 'cat-painters':
        return ['Interior room repaint', 'Drywall patch & paint', 'Ceiling water mark touchup', 'Exterior trim painting'];
      case 'cat-mechanic':
        return ['Engine check light on', 'Brake pads squeaking', 'Car battery dead', 'Oil & filter change'];
      case 'cat-hvac':
        return ['AC blowing warm air', 'Heater not turning on', 'Strange furnace odor', 'Thermostat replacement'];
      case 'cat-delivery':
        return ['Urgent document parcel', 'Fragile goods delivery', 'Express courier pickup', 'Same-day errand'];
      default:
        return ['Diagnostic inspection', 'Emergency repair', 'Maintenance service', 'New installation'];
    }
  };
  ...
  // 15 seconds timer when in searching phase
  useEffect(() => {
    ...
    if (phase === 'searching_15s' && secondsRemaining > 0) {
      timer = setInterval(() => {
        setSecondsRemaining((prev) => {
          if (prev <= 1) {
            clearInterval(timer);
            // Simulate provider acceptance if not accepted yet
  ...
  const handleStartDispatch = async () => {
    ...
    try {
      ...
      // Auto-simulate acceptance after 3.5 seconds if waiting for real provider
      setTimeout(() => {
        simulateSpecialistAccept(data.ticketId);
      }, 3500);
    } catch (e) {
      console.error('Dispatch request error:', e);
      setTimeout(() => simulateSpecialistAccept(), 3000);
    }
  };
      ...
      if (res.ok && data.success) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            senderName: currentUser?.name || 'Customer',
            text: `✅ Invoice #${invoiceId} paid successfully! Funds held in Servexa Escrow & disbursed to specialist account.`,
            mediaType: 'text',
            timestamp: new Date().toISOString(),
            read: true
          }
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {getQuickIssues(category.id).map((chip) => (
                    <button
                      key={chip}
                      type="button"
                ...
                <textarea
                  ...
                  placeholder="Describe your problem or select a common issue above..."
        ...
        {phase === 'searching_15s' && (
          <div className="p-8 sm:p-12 flex flex-col items-center justify-center text-center my-auto">
            ...
            <h3 className="text-lg sm:text-xl font-black text-slate-900 mb-1">
              Contacting {category.name} Specialists
            </h3>
            <p className="text-xs text-slate-500 max-w-sm">
              Nearby verified service providers have 15 seconds to accept your request. Connecting you immediately upon acceptance.
            </p>
            ...
            <div className="mt-8 flex items-center gap-2 text-xs text-slate-400 font-medium">
              ...
              <span>Waiting for specialist confirmation...</span>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col h-[85vh] sm:h-[680px]">
            ...
            <div className="p-3.5 sm:p-4 bg-slate-900 text-white flex items-center justify-between gap-3 shadow-md shrink-0">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                ...
                <button
                  ...
                  className="p-2 rounded-xl bg-slate-800 hover:bg-indigo-600 text-slate-200 hover:text-white transition-all cursor-pointer"
                  title="Video Call"
                >
            ...
            <div className="bg-slate-100/90 border-b border-slate-200 px-4 py-2 flex flex-wrap items-center justify-between text-[11px] text-slate-600 gap-2 shrink-0">
              ...
              <div className="flex items-center gap-2">
                <span className="font-semibold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-md border border-indigo-200 flex items-center gap-1">
                ...
                {activeInvoices.map((inv) => (
                  ...
                  >
                    <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                      ...
                      >
                        {inv.status === 'paid' ? 'Paid & Escrowed' : 'Awaiting Payment'}
                      </span>
                    </div>
                    ...
                    {inv.status === 'pending' ? (
                      ...
                      <div className="mt-2 p-2 rounded-xl bg-emerald-50 text-emerald-700 text-center text-xs font-bold flex items-center justify-center gap-1">
            ...
            {/* Invoice Creator Drawer (for provider) */}
            {showInvoiceCreator && (
            ...
            <div className="p-3 sm:p-4 bg-white border-t border-slate-200 flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 text-slate-600 transition-colors cursor-pointer shrink-0"
              ...
              <input
                ...
                className="flex-1 p-2.5 rounded-xl border border-slate-200 text-xs sm:text-sm text-slate-800 placeholder:text-slate-400 focus:outline-hidden focus:border-indigo-500"
              />
              ...
              <button
                ...
                className="p-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white transition-all shadow-xs cursor-pointer shrink-0"
              >
