...
import { X, Lock, Mail, User, Phone, Briefcase, ArrowRight, ShieldCheck } from 'lucide-react';

export const AuthModal: React.FC = () => {
  const { isAuthModalOpen, setIsAuthModalOpen, authModalTab, setAuthModalTab, login, loginWithGoogle, register } = useAuth();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div>
            <h2 className="text-lg font-extrabold text-slate-900">
              {authModalTab === 'login' ? 'Welcome to Servexa' : 'Create an Account'}
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              {authModalTab === 'login'
                ? 'Sign in to access services, bookings, and instant PBX calling'
                : 'Join as a customer or list your professional business'}
            </p>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          >
            {loading ? 'Verifying...' : authModalTab === 'login' ? 'Sign In to Servexa' : 'Create Account'}
          ...
          <button
            ...
            className="w-full py-2.5 px-4 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-50"
          >
